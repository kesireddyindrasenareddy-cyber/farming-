...
const ProgressTracker: React.FC<ProgressTrackerProps> = ({ userProfile, completedQuestsCount, totalQuestsCount }) => {
  ...
  const data = [
    { name: 'Completed', value: completedQuestsCount },
    { name: 'Remaining', value: totalQuestsCount - completedQuestsCount },
  ];

  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-2xl font-bold mb-4 text-brand-dark">Your Progress</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
        
        <div className="flex flex-col items-center justify-center p-4 bg-brand-light-green rounded-lg">
          <p className="text-sm font-medium text-brand-brown">Sustainability Score</p>
          <p className="text-5xl font-bold text-brand-green">{sustainabilityScore}%</p>
        </div>

        <div className="space-y-4">
            <div className="text-center">
                <p className="text-lg font-semibold text-brand-dark">Level {userProfile.level}</p>
                <div className="w-full bg-gray-200 rounded-full h-2.5 mt-1">
                    <div className="bg-brand-green h-2.5 rounded-full" style={{ width: `${(userProfile.points % 100)}%` }}></div>
                </div>
                <p className="text-sm text-gray-500 mt-1">{userProfile.points % 100} / 100 PTS to next level</p>
            </div>
            <div className="text-center">
                <p className="text-lg font-semibold text-brand-dark">Quests Completed</p>
        ...
        <div className="h-48">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip formatter={(value, name) => [`${value} quests`, name]} />
            </PieChart>
          </ResponsiveContainer>
        </div>
      </div>
