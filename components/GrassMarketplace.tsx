...
const GrassMarketplace: React.FC<GrassMarketplaceProps> = ({ onFind, listings, loading, error, userLocation }) => {
    ...
    return (
        <div className="p-4 border border-gray-200 rounded-lg bg-gray-50/50 h-full flex flex-col">
            <h3 className="text-lg font-bold text-brand-dark flex items-center"><GrassIcon /> <span className="ml-2">Grass Marketplace</span></h3>
            <p className="text-sm text-gray-500 mt-1 mb-4">Buy or sell field grass in your area. (+5 PTS)</p>
                        ...
                        {listings.map((listing) => (
                            <div key={listing.id} className="p-3 rounded-lg bg-white border border-gray-200">
                                ...
                                <div className="mt-3 pt-2 border-t border-gray-200">
                                     <p className="text-sm text-gray-700">Contact: <span className="font-medium">{listing.contactName}</span></p>
                                     <p className="text-sm text-gray-700">Phone: <span className="font-medium">{listing.contactPhone}</span></p>
                                     <p className="text-xs text-gray-500 mt-1">Location: {listing.location}</p>
                                </div>
                            </div>
                        ))}
                    </div>
