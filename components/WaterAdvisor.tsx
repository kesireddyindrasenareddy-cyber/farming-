...
const WaterAdvisor: React.FC<WaterAdvisorProps> = ({ onGetAdvice, result, loading, error }) => {
    ...
    return (
        <div className="p-4 border border-gray-200 rounded-lg bg-gray-50/50 h-full flex flex-col">
            <h3 className="text-lg font-bold text-brand-dark flex items-center"><WaterIcon /> <span className="ml-2">Water-Use Advisor</span></h3>
            <p className="text-sm text-gray-500 mt-1 mb-4">Get water usage advice for a crop. (+15 PTS)</p>
            ...
            <div className="mt-4 flex-grow">
                ...
                {result && (
                    <div className="p-3 rounded-lg bg-blue-50 border-l-4 border-blue-500 space-y-2">
                        <div>
                            <p className="text-xs font-semibold text-blue-800 uppercase">Amount</p>
                            <p className="text-md font-medium text-gray-800">{result.amount}</p>
                        </div>
                         <div>
                            <p className="text-xs font-semibold text-blue-800 uppercase">Frequency</p>
                            <p className="text-md font-medium text-gray-800">{result.frequency}</p>
                        </div>
                        {result.notes && result.notes.length > 0 && (
                            <div>
                                <p className="text-xs font-semibold text-blue-800 uppercase">Notes</p>
